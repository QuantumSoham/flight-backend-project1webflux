...
//
//        Query query = Query.query(Criteria.where("_id").is(flightId).and("availableSeats").gte(seatsToBook));
//        Update update = new Update().inc("availableSeats", -seatsToBook);
//        FindAndModifyOptions options = FindAndModifyOptions.options().returnNew(true);
//
//        return template.findAndModify(query, update, options, Flight.class)
//                .switchIfEmpty(Mono.error(new BadRequestException("Insufficient seats or flight not found")))
//                .flatMap((Flight updatedFlight) -> {
...
//                            .flatMap((Booking savedBooking) ->
//                                    savePassengers(savedBooking, request.getPassengers(), flightId)
//                                            .then(Mono.<Booking>just(savedBooking))
//                                            .onErrorResume(passErr ->
//                                                    rollbackBookingAndSeats(savedBooking, seatsToBook)
//                                                            .then(Mono.error(passErr))
//                                            )
//                            )
//                            .map((Booking b) -> BookingResponse.builder()
//                                    .pnr(b.getPnr())
...
public Mono<BookingResponse> bookTicket(String flightId, BookingRequest request) {
    ...
    // 2) Atomic reservation: ensure requested seats are NOT already in flight.bookedSeats and availableSeats is enough
    Query reserveQuery = Query.query(
    ...
    return template.findAndModify(reserveQuery, reserveUpdate, options, Flight.class)
            .switchIfEmpty(Mono.error(new BadRequestException("Insufficient seats or some requested seats are already taken")))
            .flatMap(updatedFlight -> {
                ...
                // Save booking then passengers. If passenger saving fails, unreserve seats and delete booking.
                return bookingRepository.save(booking)
...
private Mono<Void> unreserveSeats(String flightId, List<String> seatNumbers, int seatsToRestore) {
    ...
    Query q = Query.query(Criteria.where("_id").is(flightId));
    Update undo = new Update()
            .inc("availableSeats", seatsToRestore)
            .pullAll("bookedSeats", seatNumbers.toArray(new String[0]));
    ...
    private Mono<Void> rollbackBookingAndSeats(Booking savedBooking, int seatsToRestore) {
        ...
        return restore.then(deleteBooking).onErrorResume(e -> bookingRepository.deleteById(savedBooking.getId()).then());
    }
        ...
        return bookingRepository.findByPnr(pnr)
                ...
                .map(booking -> {
                    ...
                    return TicketDTO.builder()
                            ...
                            .flightNumber(booking.getFlightId())
